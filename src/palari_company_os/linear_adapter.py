...
class LinearClient:
    def __init__(self, api_key: str, *, endpoint: str = LINEAR_ENDPOINT, timeout: int = 20) -> None:
        if not api_key:
            raise WorkspaceError("LINEAR_API_KEY is required for Linear provider calls")
    ...
    def issue(self, identifier: str) -> dict[str, Any]:
        ...
        if not isinstance(issue, dict):
            raise WorkspaceError(f"Linear issue not found: {identifier}")
    ...
    def create_comment(self, issue_id: str, body: str) -> dict[str, Any]:
        ...
        if not isinstance(result, dict) or not result.get("success"):
            raise WorkspaceError("Linear commentCreate did not return success")
        comment = result.get("comment")
        if not isinstance(comment, dict):
            raise WorkspaceError("Linear commentCreate did not return a comment")
        return dict(comment)
    ...
    def request(self, query: str, variables: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            raise WorkspaceError(f"Linear GraphQL HTTP {exc.code}: {_short_error(raw)}") from exc
        except urllib.error.URLError as exc:
            raise WorkspaceError(f"Linear GraphQL request failed: {exc.reason}") from exc

        try:
            decoded = json.loads(raw)
        except json.JSONDecodeError as exc:
            raise WorkspaceError("Linear GraphQL response was not valid JSON") from exc
        if not isinstance(decoded, dict):
            raise WorkspaceError("Linear GraphQL response was not an object")
        ...
        if isinstance(errors, list) and errors:
            ...
            raise WorkspaceError("Linear GraphQL error: " + "; ".join(messages))
        data = decoded.get("data")
        if not isinstance(data, dict):
            raise WorkspaceError("Linear GraphQL response did not include data")
        return data
...
def parse_palari_block(description: str) -> PalariBlock:
    ...
    if match is None:
        return PalariBlock(present=False, valid=False, fields={}, error="missing palari block")
    ...
    try:
        ...
        return PalariBlock(
            ...
            valid=False,
            fields={},
            error=f"invalid palari JSON block: {exc.msg}",
        )
    if not isinstance(parsed, dict):
        return PalariBlock(
            ...
            valid=False,
            fields={},
            error="palari block must contain a JSON object",
        )
    ...
    if unknown:
        return PalariBlock(
            ...
            valid=False,
            fields={},
            error=f"unknown palari governance fields: {', '.join(unknown)}",
        )
    for field in STRING_GOVERNANCE_FIELDS:
        if field in parsed and not isinstance(parsed[field], str):
            return PalariBlock(
                present=True,
                valid=False,
                fields={},
                error=f"palari.{field} must be a string",
            )
    for field in STRING_LIST_GOVERNANCE_FIELDS:
        ...
        if value is not None and (
            ...
            return PalariBlock(
                ...
                valid=False,
                fields={},
                error=f"palari.{field} must be a list of strings",
            )
    return PalariBlock(present=True, valid=True, fields=dict(parsed))
...
def linear_send(
    ...
    *,
    ...
    issue_id, body = _comment_payload_inputs(item.payload_preview)
...
def _comment_payload_inputs(payload_preview: dict[str, Any]) -> tuple[str, str]:
