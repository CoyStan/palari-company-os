...
| EXPERIMENTAL | Cursor, Devin, GLM, and generic session profiles | Current docs label these advisory because no proven native session boundary exists. Keep outside the authoritative core gate or remove if they impose compatibility cost. |
...
| DUPLICATE | Root and packaged schema/example byte copies | SHA-256 identity and `test_public_surface`/`test_packaging` prove duplicated files. Select one source and package from it; copied data is not a second authority. |
...
| `test_agent_adoption.py` | Host installation profiles | Treat as optional adapter coverage, not governance. Reduce advisory profiles if they are removed. |
...
| `test_packaging.py` | Wheel package data | Retain as a cheap packaging boundary; stop protecting duplicate source copies. |
