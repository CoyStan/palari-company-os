...
vibes. It is not current product status, a backlog, or a compatibility
promise. Companion: the shorter [Competitive landscape snapshot](palari-blueprint.md#6-competitive-landscape-snapshot)
inside the [Palari Blueprint](palari-blueprint.md) (protocol and strategy
research). Star counts and product claims below are frozen to the July 2026
fetch window; re-verify before publication.
...
Across ~60 verified systems, **no project combines all five properties.**
Fragments of offline-verifiable or tamper-evident evidence appear in a few
middleware/audit tools, but none integrate that evidence with durable governed
roles, task contracts, and mandatory human gates the way Palari claims. The
field splits into three camps, each holding one or two organs of the concept:
...
| --- | --- | --- |
| **Microsoft Agent Governance Toolkit** | ~4.9k | The credibility threat, as middleware not a company OS. Authority: **yes** — deterministic policy engine (YAML/OPA/Cedar) with allow/deny/require-approval, the only major project architecturally separating authority from capability. Boundaries: **yes** (denies + sandbox privilege rings). Evidence: **strong partial** — Merkle/hash-chained tamper-evident audit; docs also describe an offline-verifiable decision-receipt path for governed MCP calls (Ed25519 / hash chaining). That narrows any “receipts are empty” novelty claim; AGT still lacks work items, durable governed roles, org semantics, and a local-first company loop. Gates: **yes**. Public preview. |
...
| System | Stars | Note |
| --- | --- | --- |
| **Claude Code native Agent Teams** | (Anthropic) | The commoditization event. Shared file-locked task list + mailboxes; teammates honor tool allowlists, permission prompts bubble to the human, and a teammate **cannot approve permissions or relay consent** (untrusted). Ephemeral identity, no receipts. Platform-native team execution is now a substrate Palari must compose with, not reinvent. |
